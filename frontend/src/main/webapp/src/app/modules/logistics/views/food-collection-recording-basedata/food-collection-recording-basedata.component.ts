import {Component, effect, inject, model, NgZone, ViewChild} from '@angular/core';
...
export class FoodCollectionRecordingBasedataComponent {
  ...
  selectedRouteEffect = effect(() => {
    ...
    if (selectedRoute) {
      const observer = {
        ...
        error: async (response: HttpResponse<FoodCollectionData>) => {
          if (response.status !== 404) {
            // TODO test
            this.toastService.showToast({type: ToastType.ERROR, title: 'Daten konnten nicht geladen werden!'});
          }
        },
      };
  ...
  save() {
    ...
    this.foodCollectionsApiService.saveRouteData(this.selectedRoute().id, routeData).subscribe(() => {
