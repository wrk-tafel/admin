...
* Static values manageable via UI - implemented 2026-07-27: added `GET/POST /api/settings/static-values` + `POST /api/settings/static-values/{id}` to the existing `SettingsController`/`SettingsService`, with validation rejecting `validFrom > validTo` and rejecting overlapping validity ranges (scoped per lookup method's actual key: `INCOME_LIMIT` by `(type, countAdults, countChildren)` matching `findLatestForPersonCount`; `TOLERANCE`/`ADDITIONAL_ADULT`/`ADDITIONAL_CHILD`/`CHILD_TAX_ALLOWANCE`/`COST_CONTRIBUTION` by `type` alone matching `findSingleValueOfType`, which ignores counts; `FAMILY_BONUS`/`SIBLING_ADDITION` never conflict since they're read via the list-returning `findValuesOfType`). Kept `StaticValueRepository`'s `@Cacheable` annotations and `CacheConfig` (they were added the same day this branched off, specifically to fix N+1 queries behind `getHouseholdsAboveLimit()` - dropping them would have silently undone that fix) and instead added `@CacheEvict(allEntries = true)` to the new create/update methods, so admin edits still take effect immediately without a restart. New "Statische Werte" settings page + edit dialog (all 8 `StaticValueType` values exposed via dropdown), wired into `settings.routes.ts`/nav under the existing `SETTINGS` permission; fixed a latent `testdata.sql` bug (`'INCOME_TOLERANCE'` didn't match any enum constant, so e2e/local seeded data would fail to load once read via `findAll()`); added `base::exception` to the `settings` module's Spring Modulith `allowedDependencies`. Verified end-to-end against a running backend+DB (list/create/overlap-rejection/edit), via new Cypress spec `settings-static-values.cy.ts`, and via a new integration test proving the cache is actually evicted on write (`SettingsServiceStaticValueCacheIT`).
