...
4. **Improve customer-creation / search before creating (avoid duplicates)** (merges the duplicate "enforce search before creating" entry)
...
5. **Overview: new + renewed households of last distribution**
...
7. **statistic: add alleinerzieher (single-parent) flag**
...
8. **customer-creation: postal code validation**
...
9. **Edit Route: contact-person editable + person-select dropdown with search/auto-create**
...
10. **Test if mails are properly received with mailpit rest api**
...
11. **Sec: Set cookie path to separate prod/env**
...
12. **food recording: sonstige Kisten (add description)**
...
13. **use semantic versioning + provide jar-file releases via GitHub**
...
14. **impro 5: Maybe decouple reporting from closing? Favor auto-closing** *(moved from MAYBE — the missing design piece turned out to already be answered by existing code)*
