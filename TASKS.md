...
1. **Static values (income limits, tolerance, family bonus, etc.) should be manageable via UI, not just DB migrations**
   * Today `static_values` (`StaticValueEntity.kt`) is treated as deploy-time-only data: `StaticValueRepository.kt:11-14` documents (and `CacheConfig.kt:9-12` enforces) the assumption that rows "only ever change via a DB migration ... never at runtime", so all three lookup methods (`findLatestForPersonCount`, `findSingleValueOfType`, `findValuesOfType`) are `@Cacheable` in a process-lifetime `ConcurrentMapCacheManager` (`CacheConfig.kt:20`) with no eviction. Adding a UI to edit this table without addressing that would silently keep serving stale cached values until the next process restart.
   * Needed: (a) a `StaticValueController`/`StaticValueService` under a new module (e.g. `modules/staticdata`), mirroring `SheltersController.kt`'s GET-all / `POST` create / `POST /{id}` update shape, exposing CRUD over `StaticValueEntity`'s existing columns (`type`, `validFrom`, `validTo`, `amount`, `countAdults`, `countChildren`, `age`); (b) resolve the cache staleness issue — either drop the three `@Cacheable` annotations on `StaticValueRepository.kt:15-36` (simplest, given this is a small table not on a hot enough path to need caching) or add matching `@CacheEvict`s on the new create/update endpoints. Pick one; leaving both the cache and a write path in place reintroduces exactly the staleness bug the current comment is designed to avoid.
   * Frontend: new "Statische Werte" (or similar) settings page/component under `frontend/.../modules/settings/views/`, registered in `settings.routes.ts` (alongside `email`/`notschlafstellen`) and as a new child under "Einstellungen" in `navigation-menuItems.ts:130-139`; reuse the `SETTINGS` permission already gating the parent nav entry (`navigation-menuItems.ts:129`) — no new permission needed.
   * Open question to resolve before implementing: whether to expose all 8 `StaticValueType` enum values (`StaticValueEntity.kt:40-49`) via a type dropdown, or scope the first version to just the ones most likely to change year-to-year (e.g. `INCOME_LIMIT`/`TOLERANCE`).
   * Files: new `StaticValueController.kt`/`StaticValueService.kt` (new module), `StaticValueRepository.kt` (cache annotations), `CacheConfig.kt` (comment update if caching is dropped), new frontend settings view, `settings.routes.ts`, `navigation-menuItems.ts`.

2. **Add file upload / documents to customer details** (merges both TODO occurrences)
...
3. **Improve customer-creation / search before creating (avoid duplicates)** (merges the duplicate "enforce search before creating" entry)
...
4. **Overview: new + renewed households of last distribution**
...
6. **statistic: add alleinerzieher (single-parent) flag**
...
7. **customer-creation: postal code validation**
...
8. **Edit Route: contact-person editable + person-select dropdown with search/auto-create**
...
9. **Test if mails are properly received with mailpit rest api**
...
10. **Sec: Set cookie path to separate prod/env**
...
11. **food recording: sonstige Kisten (add description)**
...
12. **use semantic versioning + provide jar-file releases via GitHub**
...
13. **impro 5: Maybe decouple reporting from closing? Favor auto-closing** *(moved from MAYBE — the missing design piece turned out to already be answered by existing code)*
