...
* Static values manageable via UI - implemented 2026-07-27: dropped `@Cacheable` from `StaticValueRepository`'s 3 lookup methods and removed the now-dead `CacheConfig`; added `GET/POST /api/settings/static-values` + `POST /api/settings/static-values/{id}` to the existing `SettingsController`/`SettingsService`, with validation rejecting `validFrom > validTo` and rejecting overlapping validity ranges for the same `(type, countAdults, countChildren)` combination (the invariant `findSingleValueOfType`/`findLatestForPersonCount` depend on to return a single row); new "Statische Werte" settings page + edit dialog (all 8 `StaticValueType` values exposed via dropdown), wired into `settings.routes.ts`/nav under the existing `SETTINGS` permission; fixed a latent `testdata.sql` bug (`'INCOME_TOLERANCE'` didn't match any enum constant, so e2e/local seeded data would fail to load once read via `findAll()`). Verified end-to-end against a running backend+DB (list/create/overlap-rejection/edit) and via new Cypress spec `settings-static-values.cy.ts`.
