...
- **The actor is resolved once**, from the `SecurityContext` of the thread that made the changes, not
  once per row — username, user id and the user's own name, all stamped onto the row so an
  entry keeps naming who made the change even after that account is renamed or deleted.
