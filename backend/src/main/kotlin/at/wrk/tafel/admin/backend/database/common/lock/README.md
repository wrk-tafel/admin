...
- `PATCH_FOOD_COLLECTION_ITEM` (4000L) - Serializes every write path that can find-or-create a food collection's row or modify its items (`saveRouteData`, `saveKm`, `saveItems`, `saveItemsPerShop`, `patchItem`) to avoid lost updates and duplicate-key races (on both the items and the `(distribution_id, route_id)` unique constraint itself) when a per-shop mobile save, a different shop's per-shop save, the desktop autosave, and the route/km forms overlap
