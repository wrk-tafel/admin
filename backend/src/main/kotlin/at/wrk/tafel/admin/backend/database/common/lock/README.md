...
- `PATCH_FOOD_COLLECTION_ITEM` (4000L) - Serializes every write path that can find-or-create a food collection's row or modify its items (`saveRouteData`, `saveKm`, `saveItems`, `saveItemsPerShop`, `patchItem`, `saveReturnItems`, `saveReturnItemsPerShop`) to avoid lost updates and duplicate-key races (on both the items and the `(distribution_id, route_id)` unique constraint itself) when a per-shop mobile save, a different shop's per-shop save, the desktop autosave, and the route/km forms overlap
...
- `SAVE_FOOD_COLLECTION_RETURN_ITEMS` (6000L) - Serializes the per-shop replace of a food collection's free-text return items, whose whole element collection is rewritten on every save. Taken nested inside `PATCH_FOOD_COLLECTION_ITEM` by both callers, not instead of it
...
- `PAY_COST_CONTRIBUTION` (10000L) - Serializes the read-modify-write of a household's pending Unkostenbeitrag in `payCostContribution`, whose lost-update race would otherwise let two operators recording partial payments at the same time silently drop one of the payments
- `LAST_ADMINISTRATOR_SAFEGUARD` (11000L) - Serializes the last-active-administrator safeguard's check-then-act, whose TOCTOU would otherwise let two concurrent deletes/disables with exactly two enabled administrators left both see "another administrator exists" and both succeed, leaving zero
...
enum class AdvisoryLockKey(val lockId: Long) {
    ...
    MY_NEW_LOCK(12000L), // Add new lock key here - existing keys are spaced 1000 apart by convention
}
```
