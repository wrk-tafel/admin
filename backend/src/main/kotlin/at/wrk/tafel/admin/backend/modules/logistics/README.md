...
- **Race condition guard, second instance:** both return-item save paths
  ...
  `advisoryLockService.withLock(AdvisoryLockKey.SAVE_FOOD_COLLECTION_RETURN_ITEMS)`, nested inside
  `PATCH_FOOD_COLLECTION_ITEM` (see below). Hibernate rewrites the whole element collection on any
  change, so a concurrent per-shop save for another shop of the same route would otherwise drop the
  rows this one just wrote.
- **Race condition guard:** `saveRouteData()`, `saveKm()`, `saveItems()`, `saveItemsPerShop()`,
  `patchItem()`, `saveReturnItems()` and `saveReturnItemsPerShop()` all wrap their find-or-create of
  the `(distribution, route)` row and/or their read-modify-write (or, for `saveItems`, outright
  replace) of `items` in the same
  ...
  the visible symptom (issue #3602). The two return-item paths were missing from this lock until
  #3628 - their own `SAVE_FOOD_COLLECTION_RETURN_ITEMS` lock only serialized them against each
  other, not against the five paths above, so a driver's per-shop return-items save could still race
  a concurrent km/route-data/items save on the same route's find-or-create. If you add another
  read-modify-write or find-or-create path against a food collection's row, it needs this lock too.
