...
internal class WebPushSenderServiceTest {
    ...
    private fun serviceRespondingWith(statusCode: Int, request: HttpPost = HttpPost("https://push.example.com/x")): WebPushSenderService {
        every { notificationFactory.create(any<Subscription>(), "{}", any()) } returns fakeNotification
    ...
    fun `send skips and reports NOT_CONFIGURED when push isn't configured`() {
        ...
        val result = service.send(testSubscription, "{}", TEST_TOPIC)
    ...
    fun `send requests the aes128gcm encoding`() {
        ...
        WebPushSenderService(pushService, notificationFactory, httpClient).send(testSubscription, "{}", TEST_TOPIC)
    ...
    fun `send drops the library's padded Crypto-Key header, which FCM rejects`() {
        ...
        service.send(testSubscription, "{}", TEST_TOPIC)
    ...
    fun `send reports EXPIRED on a 403 response`() {
        assertThat(serviceRespondingWith(403).send(testSubscription, "{}", TEST_TOPIC)).isEqualTo(PushSendResult.EXPIRED)
    }

    @Test
    fun `send reports EXPIRED on a 404 response`() {
        assertThat(serviceRespondingWith(404).send(testSubscription, "{}", TEST_TOPIC)).isEqualTo(PushSendResult.EXPIRED)
    }

    @Test
    fun `send reports EXPIRED on a 410 response`() {
        assertThat(serviceRespondingWith(410).send(testSubscription, "{}", TEST_TOPIC)).isEqualTo(PushSendResult.EXPIRED)
    }

    @Test
    fun `send reports FAILED on an unexpected status code`() {
        assertThat(serviceRespondingWith(500).send(testSubscription, "{}", TEST_TOPIC)).isEqualTo(PushSendResult.FAILED)
    }
    ...
    fun `send reports FAILED when the push library throws`() {
        ...
        assertThat(service.send(testSubscription, "{}", TEST_TOPIC)).isEqualTo(PushSendResult.FAILED)
    }
