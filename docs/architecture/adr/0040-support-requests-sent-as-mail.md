...
- The recipients are `tafeladmin.support.recipients` — operator configuration, not a UI setting
  ...
  `tafeladmin.support.subjectPrefix` ("Support:") goes in front of the reported title and marks the
  mail as a support request in a mailbox that also receives the automated ones; the environment
  stays `tafeladmin.mail.subjectPrefix`'s job, so a subject reads
