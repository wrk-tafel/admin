...
| `household_duplicate_dismissals` | customers | pairs of household numbers a reviewer judged not to be duplicates, plus the reviewer's username | deleted for a given household by `HouseholdService.deleteHouseholdByHouseholdId` - no foreign key to `households` (household numbers are never reused), so this has to be an explicit delete rather than a cascade |
