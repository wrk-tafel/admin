...
| `logs/access.log` | customers — pseudonymously by path only | one line per request, including `/api/households/{id}` paths; `server.tomcat.accesslog.pattern` deliberately omits the query string (`%m %U %H`, not Tomcat's default `%r`), and every search term travels in a `POST .../search` body rather than a query parameter in the first place, so a name has nowhere left to leak from into this file even if the pattern itself ever drifted again, see [G25](#g25-a-search-term-is-a-name-and-it-no-longer-travels-into-the-access-log-or-the-support-mail) | **never** — `rotate: false` in `application.yml` |
...
The access-log fix above regressed in production without a single line of this repository
changing: issue [#3703](https://github.com/wrk-tafel/admin/issues/3703) found real customer names
back in `access.log` weeks later, because `server.tomcat.accesslog.pattern` is bound once when
Tomcat starts and production's per-deployment settings come from an operator-managed `config.yml`
outside this repository — that file had drifted back to an older, unsafe pattern, invisible from
inside this codebase. A pattern-level fix depends on every layer between the browser and the log
file staying correctly configured forever, including one this repository cannot see or test. The
actual fix, closing the gap for good rather than patching the symptom again: the four searches (plus
the login-attempts search, the same class of leak in the same controller) no longer send the term as
a `?searchInput=...` query parameter at all — each is now `POST .../search` with the term in a JSON
body (`HouseholdSearchRequest`, `UserSearchRequest`, `LoginAttemptSearchRequest`,
`EmployeeSearchRequest`, `DataSubjectSearchRequest`). A query string that never exists cannot leak
into `access.log`, browser history or a support mail, whatever the operator's Tomcat configuration
says — see [ADR-0057](adr/0057-search-endpoints-take-their-terms-in-a-post-body.md) for why `POST` and
not the still-unreleased HTTP `QUERY` method.
...
| 24 | [G25](#g25-a-search-term-is-a-name-and-it-no-longer-travels-into-the-access-log-or-the-support-mail) search terms in `access.log`, the URL and the support mail | [#3506](https://github.com/wrk-tafel/admin/issues/3506), [#3703](https://github.com/wrk-tafel/admin/issues/3703) | done | `SupportContextService` strips the query string from `page`; the "Kunden anlegen" empty-state CTA passes the name via router state instead of query params; the access-log-pattern fix regressed via an operator `config.yml` override (#3703), closed for good by moving every search term into a `POST .../search` body (ADR-0057) |
