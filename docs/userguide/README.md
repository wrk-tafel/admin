...
Der Reiter **Meine Daten** zeigt das eigene Benutzerkonto. **Benutzername** und **Personalnummer** stehen dort nur zur Ansicht – sie werden von einem Administrator vergeben (siehe [Benutzer anlegen / bearbeiten](benutzer.md#benutzer-anlegen--bearbeiten)). **Vorname**, **Nachname** und **E-Mail** ändert man selbst und bestätigt mit **Speichern**; solange nichts geändert wurde, ist die Schaltfläche ausgegraut, und **Verwerfen** stellt die gespeicherten Werte wieder her. Die E-Mail-Adresse ist optional; an sie gehen Systembenachrichtigungen und der Code der Zwei-Faktor-Authentifizierung per E-Mail. Ist der **Code per E-Mail** eingerichtet, ist die Adresse der Empfänger Ihrer Anmelde-Codes: Zum Ändern verlangt die Seite dann zusätzlich einen aktuellen **Code** – aus der Authenticator-App oder per E-Mail (**Code per E-Mail senden** schickt ihn an die bisherige Adresse). Ein unbeaufsichtigt angemeldeter Browser kann die Adresse so nicht auf eine fremde umleiten. Die bisherige Adresse erhält außerdem eine Mitteilung über die Änderung. Jede Änderung wird wie bei jeder Benutzeränderung im [Zugriffsprotokoll](zugriffsprotokoll.md) festgehalten.
...
Nach dem Speichern bleibt man mit dem neuen Passwort angemeldet – ein neuerlicher Login ist nicht nötig. Die Anwendung bestätigt die Änderung mit einem kurzen Hinweis und leert die Felder wieder, damit die Passwörter nicht auf dem Bildschirm stehen bleiben. Das aktuelle Passwort wird dabei abgefragt; ein falsches zählt wie bei der Anmeldung als Fehlversuch und führt nach mehreren Versuchen zu einer vorübergehenden Sperre. Das eigene Passwort lässt sich nur hier ändern, nicht in der Benutzerverwaltung (siehe [Benutzer](benutzer.md#benutzer-anlegen--bearbeiten)).
...
Ab dann verlangt jede Anmeldung nach dem Passwort einen Code (siehe [Anmeldung mit Bestätigungscode](#anmeldung-mit-bestätigungscode)); auch bereits geöffnete Sitzungen auf anderen Geräten müssen den Code nachträglich bestätigen. Zum Ausschalten gibt man im Abschnitt **Ausschalten** einen aktuellen Code ein – aus der App oder, wenn eingerichtet, per E-Mail (**Code per E-Mail senden**) – und wählt **Authenticator-App ausschalten** bzw. **Code per E-Mail ausschalten**. Der Code ist nötig, damit ein unbeaufsichtigt angemeldeter Browser sie nicht ausschalten kann. Aus demselben Grund verlangt das **Einrichten einer weiteren Methode**, solange schon eine eingerichtet ist, zusätzlich einen aktuellen Code der bestehenden Methode (Feld **Aktueller Code der bestehenden Methode**, bei Code per E-Mail mit **Code per E-Mail senden**): Wer nur an einer geöffneten Sitzung sitzt, kann so keine eigene Authenticator-App oder Adresse danebenstellen. Über jede Änderung – eine eingerichtete oder ausgeschaltete Methode, eine geänderte E-Mail-Adresse oder das Zurücksetzen durch einen Administrator – informiert eine E-Mail an die Adresse des Benutzerkontos (bei einer geänderten Adresse an die bisherige), sofern der E-Mail-Versand eingerichtet ist und eine Adresse hinterlegt war. Der Schlüssel der App verlässt den Server nicht: Er steht in keiner Antwort und in keinem Export, in der Datenauskunft steht nur, ob die Zwei-Faktor-Authentifizierung aktiv ist.
...
| Daten werden bald gelöscht | Benutzerkonten (außer Administrator-Konten), Kunden oder Mitarbeiter innerhalb der nächsten 30 Tage automatisch gelöscht werden, weil ihre Aufbewahrungsfrist abläuft: ein Benutzerkonto nach einem Jahr ohne Anmeldung, ein Mitarbeiter nach einem Jahr ohne Einsatz als Fahrer:in oder Beifahrer:in, ein Kunde 7 Jahre nach Ablauf der Gültigkeit (Erinnerung jeweils in der Früh, solange es solche Datensätze gibt). Die Benachrichtigung nennt die Anzahl je Bereich und öffnet die Seite Einstellungen → Anstehende Löschungen (siehe [Einstellungen](einstellungen.md#anstehende-loeschungen)), die genau auflistet, was gelöscht wird. Eine Anmeldung der Person erhält ein Benutzerkonto, eine Verlängerung der Gültigkeit einen Kunden | Administrator |
