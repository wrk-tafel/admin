...
- Über den grünen **+**-Button bei "Aktuellste Notiz" (die Kartenüberschrift zeigt zusätzlich die Gesamtanzahl an Notizen) kann eine neue Notiz erfasst werden; sobald mindestens eine Notiz vorhanden ist, öffnet **Alle Notizen anzeigen** einen Dialog mit allen bisherigen Notizen. Die neueste Notiz zeigt zusätzlich eine relative Zeitangabe ("vor 3 Tagen") mit dem genauen Zeitpunkt als Tooltip. Im Dialog weist ein Hinweis darauf hin, nur für die Prüfung des Anspruchs notwendige Angaben festzuhalten – keine Angaben zu Gesundheit, Religion oder ähnlichen besonders schützenswerten Daten. Jede Notiz im Dialog lässt sich über den Stift-Button korrigieren oder über den Papierkorb-Button endgültig löschen (jeweils mit Sicherheitsabfrage beim Löschen) – etwa wenn sich eine Angabe im Nachhinein als falsch herausstellt oder doch besonders schützenswerte Daten enthält.

![Alle Notizen](images/kunden-alle-notizen.jpg)
