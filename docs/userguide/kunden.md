...
- **Weitere Aktionen**: Sammelt die selteneren bzw. sicherheitskritischen Aktionen **Kunde deaktivieren**, **Kunde sperren**/**entsperren**, **Daten exportieren (JSON)**, **Dokumente exportieren (ZIP)** und **Kunde löschen** (jeweils mit Sicherheitsabfrage bzw. Download) in einem Menü. Die beiden Export-Aktionen liefern die vollständige Datenauskunft zu einem Kunden für eine DSGVO-Anfrage: **Daten exportieren (JSON)** lädt Stammdaten, weitere Personen, Notizen und die Teilnahme-Historie an Ausgabetagen als eine JSON-Datei herunter, **Dokumente exportieren (ZIP)** alle hochgeladenen Dokumente gesammelt als ZIP-Datei. Beide funktionieren unabhängig vom Sperrstatus, da ein gesperrter Kunde dieselben Auskunftsrechte behält.
