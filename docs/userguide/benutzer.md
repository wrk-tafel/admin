...
Das Suchergebnis zeigt Nummer, Name, Personalnummer und Status als Chips: "Aktiv" (grün) oder "Deaktiviert" (grau), zusätzlich "Passwortänderung erforderlich" bzw. "Gesperrt bis <Datum>", falls zutreffend - letzteres macht auf einen Blick sichtbar, warum sich eine Person gerade nicht anmelden kann, ohne extra auf "Anmelde-Versuche" wechseln zu müssen. Die gesamte Zeile ist anklickbar und öffnet die Detailansicht; über den Stift in den Aktionen gelangt man direkt zum Bearbeiten, über den roten Papierkorb löscht man den Benutzer: Eine Sicherheitsabfrage nennt Benutzername und Name, und erst nach **Löschen** wird das Benutzerkonto endgültig entfernt. Die Liste lädt danach neu und bestätigt die Löschung; wird sie abgelehnt (z. B. beim letzten aktiven Administrator), erscheint die Begründung als Fehlermeldung. Von dem Benutzer angelegte Kunden und Notizen bleiben erhalten und zeigen "Mitarbeiter gelöscht". Dieselbe Sicherheitsabfrage erscheint auch bei **Benutzer löschen** in den Benutzerdetails. Liefert die Suche keine Treffer, erscheint statt des Ergebnisses der Hinweis "Keine Benutzer gefunden" mit einem Button **Benutzer anlegen**; bei vielen Treffern kann über die Seitennavigation geblättert werden. Ein Klick auf eine der Spaltenüberschriften Nr., Name, Personalnummer oder Status sortiert die Tabelle danach statt nach den besten Treffern zuerst; ein erneuter Klick auf dieselbe Spalte kehrt die Richtung um.

![Benutzer löschen](images/benutzer-loeschen.jpg)
...
Die Detailansicht zeigt Name, Benutzername, E-Mail-Adresse (ein Strich, falls keine hinterlegt ist), Personalnummer, ob die Zwei-Faktor-Authentifizierung aktiv ist (und mit welcher Methode: Authenticator-App, Code per E-Mail oder beiden), ob eine Passwortänderung erforderlich ist, den Aktiv-Status sowie alle zugewiesenen Berechtigungen, gruppiert nach Kategorie (Ausgabe & Betrieb, Logistik, Leitung, Verwaltung); eine Kategorie, in der der Benutzer keine einzige Berechtigung hat, wird gar nicht erst angezeigt.
...
Ist die Zwei-Faktor-Authentifizierung des Benutzers aktiv, steht in der Zeile **Zwei-Faktor-Authentifizierung** neben dem Status die Schaltfläche **Zurücksetzen**: Das schaltet alle Methoden ohne Code aus – für jemanden, der das Handy mit der Authenticator-App verloren hat; danach genügt wieder das Passwort (ist die Zwei-Faktor-Authentifizierung für die gesamte Anwendung vorgeschrieben, muss der Benutzer sofort wieder eine Methode einrichten), und der Benutzer kann sie selbst neu einrichten (siehe [Anmeldung](README.md#anmeldung)). Das Zurücksetzen bei einem Administrator-Konto ist – wie das Zurücksetzen seines Passworts – nur einem Administrator erlaubt.
...
Über **Daten exportieren (ZIP)** lässt sich eine DSGVO-Datenauskunft (Art. 15/20) zu diesem Benutzerkonto als ZIP-Datei herunterladen – Stammdaten (Benutzername, Personalnummer, Name, E-Mail-Adresse, Aktiv-Status, Erstellungsdatum, letzter Login), alle zugewiesenen Berechtigungen (jeweils mit Datum und Person der Erteilung), registrierte Push-Geräte, individuell angepasste Benachrichtigungseinstellungen, offene Anmeldeversuche sowie die Login-Historie der letzten 30 Tage, sowohl als PDF-Datei als auch als maschinenlesbare JSON-Datei. Nützlich, wenn eine Auskunftsanfrage im Namen einer anderen Person gestellt wird, etwa durch die Personalabteilung oder nach dem Ausscheiden aus dem Verein. Für die eigenen Daten steht derselbe Export auch in **Mein Konto** zur Verfügung (Benutzer-Icon oben rechts, **Mein Konto**, Reiter **Datenschutz**, **Meine Daten exportieren**).
...
Beim Anlegen bzw. Bearbeiten werden Benutzername, Personalnummer, Nachname, Vorname und E-Mail-Adresse erfasst. Benutzername, Personalnummer, Nach- und Vorname sind Pflichtfelder (max. 50 Zeichen); die Personalnummer darf nicht bereits bei einem anderen Benutzer vergeben sein. Die **E-Mail-Adresse** ist optional (max. 255 Zeichen) und dient dazu, Systembenachrichtigungen und Bestätigungscodes der Zwei-Faktor-Authentifizierung an diesen Benutzer schicken zu können; wird sie angegeben, muss sie ein gültiges E-Mail-Format haben, sonst erscheint direkt am Feld "E-Mail-Format ungültig" und "Speichern" bleibt gesperrt. Wird das Feld geleert, ist danach keine Adresse mehr hinterlegt. Der Speichern-Button ist erst aktiv, sobald alle Pflichtfelder gültig ausgefüllt sind, und bleibt am unteren Bildschirmrand sichtbar ("sticky"), auch wenn weiter oben in der (teils langen) Berechtigungsliste gescrollt wird.

Das **eigene Benutzerkonto** lässt sich hier nur eingeschränkt ändern: Statt des Abschnitts **Passwort zurücksetzen** steht ein Hinweis, dass das eigene Passwort unter **Mein Konto** > **Passwort** geändert wird (dort wird das aktuelle Passwort abgefragt), und ist der **Code per E-Mail** eingerichtet, ist auch die eigene **E-Mail-Adresse** gesperrt – sie wird unter **Mein Konto** > **Meine Daten** geändert, wo ein Code verlangt wird. Die E-Mail-Adresse eines Administrator-Kontos kann – wie Benutzername, Passwort und die Passwortänderungs-Pflicht – nur ein Administrator ändern; für alle anderen ist das Feld gesperrt.

Personalnummer, Nachname und Vorname gehören zum Benutzerkonto selbst und werden direkt eingetragen. Ein Benutzerkonto ist vom Mitarbeiter-Datensatz (Fahrer:innen, siehe [Mitarbeiter](einstellungen.md#mitarbeiter)) völlig getrennt: Es gibt keine Verknüpfung, und eine Änderung hier wirkt sich nicht auf Mitarbeiter aus – und umgekehrt. Ist jemand sowohl Benutzer als auch Fahrer, sind das zwei Datensätze, die bei Bedarf beide gepflegt werden.
...
Die Berechtigung **Datenauskunft** gibt Zugriff auf die gleichnamige Seite (siehe [Datenauskunft](datenauskunft.md)), auf der sich Kunden, Benutzerkonten und Mitarbeiter mit einem einzigen Suchfeld finden lassen. Sie kommt **zusätzlich** zur jeweiligen Fachbereichs-Berechtigung hinzu, ersetzt diese aber nicht: Wer dort einen Kunden-Treffer exportieren oder löschen möchte, braucht weiterhin "Kundenverwaltung", bei einem Benutzerkonto "Benutzerverwaltung" und bei einem Mitarbeiter "Einstellungen" – genau wie beim direkten Aufruf über die jeweilige Seite.
