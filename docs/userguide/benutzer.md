...
Das Suchergebnis zeigt Nummer, Name, Personalnummer und Status (Aktiv). Über die Aktionen kann der Benutzer angesehen (Lupe) oder bearbeitet (Stift) werden.
...
Über **Benutzer bearbeiten** gelangt man zur Bearbeitungsmaske, über **Benutzer-Status ändern** kann der Benutzer aktiviert/deaktiviert werden.

## Benutzer anlegen / bearbeiten

Beim Anlegen bzw. Bearbeiten werden Benutzername, Personalnummer, Nachname, Vorname sowie optional ein Passwort erfasst (über **Passwort generieren** kann automatisch ein sicheres Passwort erzeugt werden). Zusätzlich kann festgelegt werden, ob der Benutzer aktiv ist und ob beim nächsten Login eine Passwort-Änderung erzwungen wird.
...
| Verwaltung | Kunden über dem Limit, Kunden-Duplikate, Statistiken |
